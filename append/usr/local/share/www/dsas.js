...
// Declare the function of this code that are referenced in other HTML
// files or in strings and eslint doesn't detect these uses
/* exported dsas_loggedin dsas_login dsas_display_passwd dsas_display_passwd
 * exported dsas_change_passwd dsas_status dsas_check_warnings dsas_togglelogs
 * exported dsas_display_logs dsas_user_passwd dsas_real_user_passwd
 * exported dsas_user_delete dsas_real_user_delete dsas_user_new dsas_real_user_new
 * exported dsas_change_users dsas_renew_cert dsas_renew_cert_real dsas_upload_crt
 * exported dsas_change_net dsas_display_service dsas_change_service dsas_cert_delete
 * exported dsas_cert_real_delete dsas_pubkey_name dsas_upload_cert dsas_task_delete
...
function dsas_display_users(){
    fetch("api/dsas-users.php").then(response => {
        ...
        var body = "";
        for (const user of (users.constructor === Object ? [users] : users)) {
...
function dsas_change_users(){
    fetch("api/dsas-users.php").then(response => {
        ...
        for (const user of (users.constructor === Object ? [users] : users)) {
            ...
            var active = (document.getElementById("active_" + username).checked ? "true" : "false");
...
function dsas_display_web(what = "all"){
    fetch("api/dsas-web.php").then(response => {
        ...
        if (what === "all" || what === "cert") {
            ...
            var validity = parseInt(web.ssl.validity);    
            for (let i = 1; i <= 5; i++) {
                if (validity  === i)
    ...
    fetch("api/dsas-net.php").then(response => {
        ...
        if (what === "ifaces" || what === "all") {
           ...
           "  <div class=\"container p-3 my-3 border\" id=\"IFaces\">";
            for (const iface2 of ["bas", "haut"]) {
                var iface = net[iface2];
                body = body +
           ...
           "</div></form>";  
            ...
            for (iface of [net.bas, net.haut]) { 
                ...
                if (! empty_obj(iface.dns.nameserver))
                    for (const ns of (iface.dns.nameserver.constructor === Array ? 
       ...
       document.getElementById("iface_nameserver" + i).disabled =  
        ...
        fetch("api/dsas-net.php").then(response => {
            ...
            for (const iface2 of ["bas", "haut"]) {
                ...
                var server = [];
                for (var s of document.getElementById("iface_nameserver" + j).value.split(/((\r?\n)|(\r\n?))/)) {
                    s = (s ? s.trim() : "");
                    if (s) {
                        server.push(s);
                    }
    ...
    fetch("api/dsas-service.php").then(response => {
        ...
        if (what === "ntp" || what === "all") {
            ...
            if (! empty_obj(serv.ntp.server)) {
                for (const s of (serv.ntp.server.constructor === Array ? 
...
function dsas_change_service(what) {
    if (what === "ssh") {
        ...
        fetch("api/dsas-service.php").then(response => {
            ...
            var server = [];
            for (var s of document.getElementById("ntp_pool").value.split(/((\r?\n)|(\r\n?))/)) {
                s = (s ? s.trim() : "");
                if (s) {
                    server.push(s);
                }
...
function treat_gpg_certs(certs) {
    ...
    for (const cert of certs) {
        ...
        var name = cert.uid;
        var cls = (cert_expired(cert) ? "text-danger" : (cert_expiring(cert) ? "text-warning" : "text"));
...
function treat_x509_certs(certs, added = false) {
    ...
    for (const cert of certs) {
        var pemblob = new Blob([cert.pem], {type : "application/x-x509-user-cert"});
        var url = window.URL.createObjectURL(pemblob);
        var name = cert_name(cert);
        var cls = (cert_expired(cert) ? "text-danger" : (cert_expiring(cert) ? "text-warning" : (cert_is_ca(cert) ? "text" : "text-info")));
...
function cert_body(cert) {
    ...
    var purposes = [];
    for (const key of Object.keys(cert.purposes))
